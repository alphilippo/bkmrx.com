$(document).ready(function(){$('[data-toggle="modal"]').click(function(e){e.preventDefault();var href=$(this).attr('href');if(href.indexOf('#')==0){$(href).modal('open');}else{$('#bkx-modal').modal('show');$('#bkx-modal').load(this.href);}});});$('#filter').keyup(function(){updateTags($('#filter').val());});function updateTags(tagname,param){$.ajax({url:'/ajax/json_taglist',data:{filter:tagname,exists:param},dataType:"json",type:'POST',success:function(result){var items=[];$.each(result,function(arr,hash){$.each(hash,function(key,val){var href='/bkmrx/'+key;if(param=='y'){href='/bkmrx/'+tagname+'|'+key;}
items.push('<li><a href="'+href+'"><i class="fa fa-tag"></i> '+key+'</a> <sup>'+val+'</sup></li>');});});$('#ajaxtags').html(items.join(''));}});}
