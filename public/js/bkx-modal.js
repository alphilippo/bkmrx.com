    	...
    	$.ajax({  
    	  ...
    	  success: function(result) {  
    		...
    		if (result.status == 'error') {
    			$('#url_msg').append("There was an error fetching your URL");
    		}
    		if (result.url != result.orig_url) {
    			$('#url_msg').append("Your URL was redirected. You can choose the original or the redirected URL below");
    			$('div#callback').append("<div class='form-group'><select name='url' class='form-control'><option selected>" + 
    									result.url + '</option><option>' +
    									result.orig_url + '</option></select></div>');
    		} else {
    			$('#ret_url').append(result.url);
