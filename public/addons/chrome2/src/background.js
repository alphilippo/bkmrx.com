...
redditInfo = {
  ...
  update: function(callback) {
    this.request({
      url: 'http://localhost:3000/api/me.json',
  ...
  fetchMail: function(callback) {
    this.request({
      url: 'http://localhost:3000/message/unread.json',
      success: function(resp) {
        if (resp.data) {
  ...
  _queryInfo: function(params, callback) {
    ...
    params.limit = 1
    this.request({
      url: 'http://localhost:3000/api/info.json',
      data: params,
      success: function(resp) {
        if (resp.data) {
          this.storeModhash(resp.data.modhash)
...
mailNotifier = {
  ...
  showNotification: function(title, text) {
    ...
    this.notification.onclick = function() {
      window.open('http://localhost:3000/message/unread/')
      n.cancel()
    }
